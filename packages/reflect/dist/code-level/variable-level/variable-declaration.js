...
import { CodePiece, CodePieceType, ReflectLink, Identifier, TypeLevel, AstNodeTraits } from "../index.js";
export class VariableDeclaration {
    ...
    getIdentifier = () => {
        ...
        if (children.length === 0) {
            return Result.fail(`The variable statement has no identifier`, `Please update the VariableStatement class to support '${this._tsNode.getText()}' variable statement identifier fetching`);
        }
    ...
    getAstIdentifiers = async () => {
        ...
        if (!AstNodeTraits.isChildExist(this._tsNode, 0)) {
            return Result.fail(`The '${this._tsNode.getText()}' is empty`, `Please update to have the data in the variable statement`);
        }
        ...
        if (!Identifier.isA(identifier)) {
            if (VariableDeclaration.isObjectBindingPattern(identifier)) {
                ...
                for (let i = 0; i < objectBindings.length; i++) {
                    ...
                    const refNode = CodePiece.fromTsNode(binding[0]);
