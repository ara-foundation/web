 ...
 */
import { parse as AstroParse, transform } from "@astrojs/compiler";
...
(function (FileExtension) {
    ...
    FileExtension["DirectoryOrUndefined"] = "";
...
const absolutePath = (modulePath, glob) => {
    // Astro framework adds the absolute file paths.
    let filePath = glob.file;
 ...
 * @returns
 */
export const globToUiContent = async (modulePath, glob) => {
    ...
    if (fileExtension === FileExtension.DirectoryOrUndefined) {
        return Result.fail(`The path is directory or undefined`, `Ara Web doesn't support '${absoluteModulePath}' file path derived from '${modulePath}' module path`);
    }
    const fileContent = {
        absoluteModulePath,
        fileExtension,
 ...
 */
const parseAstroFile = async (fileContent, astroSource) => {
    if (fileContent.fileExtension !== FileExtension.Astro) {
        return fileContent;
    }
 ...
 */
const extractAstroComponents = (ast) => {
 ...
 */
const astroToTs = async (fileName, astroSource) => {
