...
const identifyImportLink = (astImport: ImportDeclaration): Result<AraLink<string>> => {
    for (let child of astImport.getChildren()) {
        if (isNonImportantNode(child) || isOneOfIdentifiers(child, ["import", "from"])) {
            continue;
        } else if (child instanceof Identifier) {
        ...
        if (child instanceof ImportClause) {
                ...
                return Result.ok(new AraLink(PurlProtocol, StringTraits.unquote(child.getText()), AraWebModuleSlugs));
            } else {
                Debug.log(
                    `The astimport '${astImport.getText()}' has a '${child.getText()}' child that is not import clause:`
                )
...
export const identifyNamedImports = (astImport: ImportDeclaration, importPath: AraLink<string>): Result<Identifiers> => {
