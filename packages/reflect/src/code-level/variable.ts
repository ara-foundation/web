        ...
        for (let j = 1; j < varDeclaration.getChildCount(); j++) {
            ...
            if (AstNode.tsNodeIsNonImportant(child)) {
                ...
                if (child instanceof ArrayTypeNode) {
                    ...
                    const typeRefAraLink = ReflectAraLink.linkToIdentifier(typeRefIdentifier.getText(), {'type': 'array'});
...
export const defineVariableDeclaration = (varStatement: VariableStatement, memory: ProjectMemory): Result<AstIdentifiers> => {
    ...
    for (let i = 0; i < childCount; i++) {
        ...
        if (AstNode.tsNodeIsNonImportant(varChild)) {
            ...
            const identified = identifyVariableDeclarationList(varChild, astNode, memory);
            // Debug.pop();
            if (identified.isFailure) {
                return Result.fail(
                    `identifyVariableDeclarationList(varDeclaration='${varChild.getText()}', identifier='${JSON.stringify(astNode)}'): ${identified.errorTitle}`,
