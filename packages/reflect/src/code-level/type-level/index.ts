    ...
    public static getTypeIdentifiers = async (tsNodes: Node[]): Promise<Result<CodePiece[]>> => {
        ...
        for (let tsNode of typeStatements) {
            ...
            if (typeStatement.isFailure) {
                return Result.fail(
                    `TypeDeclarationTraits.fromTsNode(tsNode: '${tsNode.getText()}'): ${typeStatement.errorTitle}`,
                    typeStatement.errorDescription!
                )
            }
            ...
            if (identifiedTypeDeclaration.isFailure) {
                return Result.fail(
                    `TypeDeclaration('${typeStatement.getValue().getText()}'): getAstNode(): ${identifiedTypeDeclaration.errorTitle}`,
