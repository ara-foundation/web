...
import type { IdentifiedNodeDataType, ValueType } from "../code-level/ast-node-data.js";
...
export const expectValidVariableNode = (astNode: AstNode, identfier: string, constantFlag: boolean, publicFlag: boolean): void => {
    ...
    expect(ReflectAraLink.isExpressionLink(astNode.data as AraLink<Node>)).toBe(true);
    expect(astNode.dataType).toBeUndefined();
