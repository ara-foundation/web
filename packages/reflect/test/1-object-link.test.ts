...
import { DOCUMENT_SELECTOR, ModuleLink, Rest, ExtensionOperator, Setup } from "@ara-web/sds";
import { MEMOP_TAG, MODULE_MEMORY_TAG, Module, ChiefModuleManager, reflectDataToObjectTree, type ReflectDataType } from "../src/index";
...
//     const restOptions = {packageLink: pkgLink, extensions: [projectMemory.restHandler]};
//     const rest = new Rest<any>(reflectDataToObjectTree, restOptions);
...
test(`Modules added after rest instance, must work`, async() => {
    ...
    const restOptions: Setup = {
        proxies: [restProxy],
        extensions: [projectMemory.restHandler],
        packageLink: pkgLink
    }
    const restRaw = new Rest<any>(reflectDataToObjectTree, restOptions);
