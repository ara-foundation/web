...
import { MEMOP_TAG, MODULE_MEMORY_TAG, ModuleManager, ModuleMemory, ModuleMemoryOperator, Reflect, reflectDataToObjectTree, type ReflectDataType } from "../src/index";
...
const elementType = (element: ReflectDataType): string => {
    if (element instanceof CodePiece) {
        ...
        return `Module Memory('${element.moduleLink.url}')`
    } else if (element instanceof ModuleMemoryOperator) {
...
//     const reflectExtOperator = new RestfulExtensionOperator(pkgLink, MEMOP_TAG, extOperator);
//     const projectMemory = new ProjectMemory(reflectExtOperator);
//     // The child nodes of project memory.
//     // It should be:
//     // #document -> memop#pkg
//     // #document -> memop#pkg
//     const builtInExt1 = new NodejsModuleManager();
//     const builtInExt2 = new NodejsModuleManager();
...
//     const restOptions = {packageLink: pkgLink, extensions: [reflectExtOperator.restDispatcher]};
...
//     const restfulLinked = await reflectExtOperator.setRestDispatcherOperator(rest);
...
//     const firstModuleLink = modulesPutted.getValue()[0].url
...
//     const firstModule = firstModuleNode?.data! as ModuleMemory<unknown>;
...
//     const builtInExt2 = new Reflect({packageLink: ModuleLink.newPackageLink('', 'reflect2')}).nodeJsExt
...
test(`Modules added after rest instance, must work`, async() => {
    ...
    const extOperator = new ExtensionOperator([]);
    const projectMemory = new ModuleMemoryOperator(extOperator);
    // The child nodes of project memory.
    // It should be:
    // #document -> memop#pkg
    // #document -> memop#pkg
