...
import { CodePiece, CodePieceType, type CodePieceFilter, type TypedData } from "./code-piece.js";
...
import { ChiefModuleManager, FilePath } from "@ara-web/reflect";
...
export class Code {
     ...
     */
    public getImportedIdentifiers = async (projectMemory: ChiefModuleManager): Promise<Result<CodePiece[]>> => {
     ...
     */
    private importClauseToModuleLink = (importClause: string, callingModulePath: ModuleLink, projectMemory: ChiefModuleManager): Result<ModuleLink> => {
    ...
    private setImportPaths = (importModuleLink: ModuleLink, defaultIdentifier: string|undefined, astIdentifiers: CodePiece[]): CodePiece[] => {
     ...
     * moduleMemory -> codePieces
     * const identifierNodes = await moduleMemory.rest.getAll!(MODULE_SELECTOR);
     * @param moduleMemory 
     * @param projectMemory {Lint from all modules}
     * @returns 
     */
    public getLintedImportIdentifiers = async <T>(codePieces: CodePiece[], projectMemory: ChiefModuleManager): Promise<Result<CodePiece[]>> => {
        const identifiers = codePieces.filter(CodePiece.isDefinedInOtherModule)
     ...
     * @returns 
     */
    private identifyImportedIdentifier = async(identifiedNode: CodePiece, memory: ChiefModuleManager): Promise<Result<CodePiece>> => {
        ...
        const glob = identifiedMemory.getValue().namespaceObject;
    ...
    // Pass the BuiltInIdentifiers.isNonBuiltInIdentifier as skipFilter
    public getLintedTypeIdentifiers = async <T>(codePieces: CodePiece[], skipFilter: CodePieceFilter): Promise<Result<CodePiece[]>> => {
        const typesToLint = codePieces
            .filter(CodePiece.isDefinedInLocal)
            .filter(CodePiece.isTypeDeclaration)
            .filter(skipFilter);

        const typesCount = typesToLint.length;
        if (typesCount == 0) {
            return Result.ok(codePieces);
        }

        typesToLint.forEach((node, index, arr) => {
            if (typeof (node as CodePiece).data === "string") {
                arr[index].dataType = (node as CodePiece).data as ValueTypeString;
                return
            }

            const moduleIdentifiers = codePieces
                .filter(CodePiece.isTypeDeclaration)
            ...
            if (lintedNode.isFailure) {
                return Result.fail(
                    `TypeLevel.lintType(node: '${node.identifier}'): ${lintedNode.errorTitle}`,
     ...
     * @returns 
     */
    public getTypeIdentifiers = async (): Promise<Result<CodePiece[]>> => {
        const tsNodes = this.getTsNodes();
    ...
    public getVariableIdentifiers = async (): Promise<Result<CodePiece[]>> => {
        const tsNodes = this.getTsNodes();
    ...
    public getLintedVariableIdentifiers = async <T>(codePieces: CodePiece[], skipFilter: CodePieceFilter): Promise<Result<CodePiece[]>> => {
        const varsToLint = codePieces
            .filter(CodePiece.isDefinedInLocal)
            .filter(CodePiece.isVariableDeclaration)
            .filter(skipFilter);
     ...
     */
    public static identifyCodePiece = async (expression: string, skipFilter: CodePieceFilter, optionalIdentifiers?: CodePiece[]): Promise<Result<TypedData>> => {
        const tempModuleLink = ModuleLink.newFileLink(import.meta.filename);
        const tempVarName = "__temp_var_";
        const code = new Code(`const ${tempVarName} = ${expression}`, tempModuleLink);
        ...
        if (optionalIdentifiers) {
            codePieces.push(...optionalIdentifiers);
        }
        codePieces.push(...vars.getValue());
        ...
        const expressionIdentified = await code.getLintedVariableIdentifiers(codePieces, skipFilter);
