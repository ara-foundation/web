...
import { callFuncInModule, fileContentByModulePath, identifierInModule } from "@scripts/reflect/fileLevel";
import { unquote } from "@scripts/string";
import { Result } from "@scripts/result";
import { Debug } from "@scripts/debug";
import { type ValueType, type Identifiers, type IdentifiedNode, AstNodeType, type EnumMembers, type TypeDeclaration, type IdentifiedNodeDataType, ValueTypeString } from "@scripts/reflect/codeLevel/types";
import { Memory } from "@scripts/reflect/codeLevel/memory";
import { identifyImportDeclarations } from "@scripts/reflect/codeLevel/importDeclaration";
import { defineVariableDeclaration } from "@scripts/reflect/codeLevel/variable";
import { isExportKeyword, isNonImportantNode, isOneOfIdentifiers, isTypeKeyword } from "@scripts/reflect/codeLevel/astNode";
import { araIdentifierLink, araWebModuleLink, isAraExpressionLink, isAraIdentifierLink } from "@scripts/aralink/rpc";
import { AraLink } from "@scripts/aralink/types";
import { deepCopy } from "./painPointFixes";
...
export class Code {
     ...
     */
    constructor(code: string, tempCodeAmount = 0, memory?: Memory) {
     ...
     */
    private clone = (): this => {
        const cloned = new (this.constructor as typeof Code)(this.code, this.tempCodeAmount, this._memory) as this;
        return cloned;
    }
    ...
    private identifyImportDeclarations = (): Result<Identifiers> => {
    ...
    private lintImportedIdentifiers = async (identifiers: Identifiers): Promise<Result<Identifiers>> => {
        for (let identifier in identifiers) {
            ...
            if (identifiedValue.getValue().data === undefined) {
                const err = Debug.error(
                    ...
                    `Update the lintImportedIndetifiers() to supported it`,
    ...
    private defineVariableDeclarations = (memory: Memory): Result<Identifiers> => {
    ...
    private identifyTypeDeclarations = (memory: Memory): Result<Identifiers> => {
        let identifiers: Identifiers = {};
        for (let child of this.ast.getChildren()) {
            for (let i = 0; i < child.getChildCount(); i++) {
                const subChild = child.getChildAtIndex(i)
                ...
                let identifiedNode: IdentifiedNode = {
                ...
                for (let i = 0; i < childCount; i++) {
                    ...
                    if (isExportKeyword(typeChild)) {
                        ...
                        identifier = unquote(typeChild.getText());
    ...
    private isImportedNode = (node: IdentifiedNode): boolean => {
        return (node.importPath !== undefined)
    }
     ...
     * @returns 
     */
    private lintTypeDeclarations = async (memory: Memory): Promise<Result<undefined>> => {
        ...
        for (let typeIdentifierIndex = 0; typeIdentifierIndex < typeIdentifiersCount; typeIdentifierIndex++) {
            ...
            for (let typeProperty in data) {
                if (data[typeProperty] instanceof AraLink) {
                    if (!isAraIdentifierLink(data[typeProperty])) {
                        return Result.fail(
                    ...
                    const typeNode = memory.identifierByAraLink(data[typeProperty]);
    ...
    private lintVariables = async (memory: Memory): Promise<Result<undefined>> => {
        ...
        for (let varIdentifierIndex = 0; varIdentifierIndex < varIdentifiersCount; varIdentifierIndex++) {
            ...
            if (!isAraExpressionLink(varIdentifier.data as AraLink)) {
                Debug.log(varIdentifier.data)
                    return Result.fail(
     ...
     * @returns 
     */
    private identifyVariableValues = async (memory: Memory): Promise<Result<undefined>> => {
        const varIdentifiers = memory.identifiersByType(AstNodeType.Variable)
        const varIdentifiersCount = varIdentifiers.length;
        Debug.log(`Lint '${varIdentifiersCount}' variable identifiers`);
        for (let varIdentifierIndex = 0; varIdentifierIndex < varIdentifiersCount; varIdentifierIndex++) {
    ...
    private identifyMemory = async (): Promise<Result<Memory>> => {
     ...
     */
    private identifyVariableUpdates = async (identifier: string, data: ValueType|undefined, dataType: ValueTypeString|ValueType, memory: Memory): Promise<Result<ValueType>> => {
     ...
     */
    private identifyVariableAssignments = async (identifier: string, data: ValueType|undefined, dataType: ValueTypeString|ValueType, memory: Memory): Promise<Result<ValueType>> => {
    ...
    private identifyExpressionStatement = async(identifier: string, data: ValueType|undefined, dataType: ValueTypeString|ValueType, exp: ExpressionStatement, memory: Memory): Promise<Result<ValueType>> => {
    ...
    private identifyBinaryExpression = async (identifier: string, data: ValueType|undefined, dataType: ValueTypeString|ValueType, exp: BinaryExpression, memory: Memory): Promise<Result<ValueType>> => {
     ...
     * @returns 
     */
    private identifyImportedIdentifier = async<T extends ValueType>(identifiedNode: IdentifiedNode): Promise<Result<IdentifiedNode>> => {
     ...
     */
    private identifyIdentifierRecursively = async <T extends ValueType>(identifier: string, memory: Memory): Promise<Result<IdentifiedNode>> => {
        ...
        if (enumRes.isFailure) {
            ...
            if (sourceCode !== undefined) {
                ...
                const identified: IdentifiedNode = {
     ...
     * @param syntaxList 
     */
    private identifyObjectLiteral = async(identifier: string|undefined, data: ValueType|undefined, dataType: ValueTypeString|any, syntaxList: SyntaxList, memory: Memory): Promise<Result<ValueType>> => {
     ...
     * @returns 
     */
    private identifyDataType = (dataType: IdentifiedNodeDataType, data: ValueType, memory: Memory): Result<ValueTypeString|any> => {
        ...
        const dataTypeLink = dataType as AraLink;

        if (!isAraIdentifierLink(dataTypeLink)) {
            return Result.fail(
    ...
    private identifySyntaxList = async (nodes: Node[], dataElements: (ValueType)[], dataElementTypes: (ValueTypeString|any)[], memory: Memory): Promise<Result<(ValueType)[]>> => {        
    ...
    private identifyArrayExpression = async (syntaxList: SyntaxList, data: ValueType|undefined, dataType: ValueTypeString|undefined|any, memory: Memory): Promise<Result<ValueType[]>> => {
     ...
     * @returns 
     */
    private identifyValue = async (identifier: string|undefined, data: ValueType|undefined, dataType: ValueTypeString|undefined|any, exp: Node, memory: Memory): Promise<Result<ValueType>> => {
        ...
        if (exp instanceof ObjectLiteralExpression) {
            ...
            if (exp.getText() === "undefined") {
                ...
                Debug.pop();
                return Result.ok(araIdentifierLink(exp.getText(), this.dataTypeToLinkProperties(dataType)))
            ...
            const identified = await this.identifyIdentifierRecursively<typeof propertyValue>(propertyIdentifier.getText(), memory);
            Debug.pop();
    ...
    private identifyMethodCall = async(method: PropertyAccessExpression, methodArgs: SyntaxList, memory: Memory): Promise<Result<ValueType>> => {
     ...
     * @returns {error?: string, data?: T}
     */
    private identifyFunctionCall = async(exp: CallExpression, memory: Memory): Promise<Result<ValueType>> => {
     ...
     * @returns {error?: string, data?: T}
     */
    private identifyVariable = async (identifier: string, memory: Memory, identifyUpdates: boolean = true): Promise<Result<ValueType>> => {
